...
def infer_task_signals(task_text: str) -> TaskSignals:
    ...
    paths = re.findall(
        r"(?:[A-Za-z]:[\\/]|/|\b(?:src|tests?|lib|app)[\\/])[^\s,，;；)）]+", text
    )
    ...
    needs_verification = bool(
        re.search(
            r"运行(?:全量|完整|回归)?测试|跑(?:全量|完整|回归)?测试|验证|回归|test(?:s|ing)?",
            text,
            re.I,
        )
    )
    needs_recovery = bool(
        re.search(
            r"失败后|如果失败|出错后|重试|重新运行|重新验证|retry|recover", text, re.I
        )
    )
...
def _format_chain(selected: list[str]) -> str:
    ...
    while i < len(selected):
        if (
            selected[i] == "edit_file"
            and i + 1 < len(selected)
            and selected[i + 1] == "write_file"
        ):
            rendered.append("edit_file 或 write_file")
            i += 2
            continue
        if (
            selected[i] == "batch_edit"
            and i + 1 < len(selected)
            and selected[i + 1] == "edit_file"
        ):
