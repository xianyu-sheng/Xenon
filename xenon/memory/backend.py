...
class MemoryBackend(Protocol):
    ...
    def save_records(
        self, records: list[MemoryRecord], *, render: bool = True
    ) -> None: ...
    def mutate_records(
        self,
...
class JsonMarkdownBackend:
    ...
    def _render_views(self, records: list[MemoryRecord]) -> None:
        ...
        grouped: dict[MemoryKind, list[MemoryRecord]] = {
        ...
        for kind, filename in _KIND_FILES.items():
            ...
            items = sorted(
                grouped[kind], key=lambda item: item.updated_at, reverse=True
            )
            title = kind.value.replace("-", " ").title()
            lines = [
                f"# {title}",
                "",
                "<!-- Generated by Xenon. Edit through Xenon memory commands. -->",
                "",
            ]
            for item in items:
        ...
        for kind, filename in _KIND_FILES.items():
            ...
            if count:
                index_lines.append(
                    f"- [{kind.value.title()}]({filename}) — {count} active"
                )
