...
- command: {{"action": "终端命令"}} — 执行终端命令
- read_file: {{"file_path": "文件路径"}} — 读取本地文件
- write_file: {{"file_path": "文件路径", "content": "文件内容"}} — 写入本地文件
- list_files: {{"file_path": "目录路径", "pattern": "*.py"}} — 列出本地目录文件
- search_files: {{"file_path": "搜索目录", "search_pattern": "关键词"}} — 搜索文件内容
- git: {{"git_command": "status|diff|log|add|commit"}} — Git 操作
- web_fetch: {{"url": "网址"}} — 抓取任意网页内容（HTML自动转文本）
- github_fetch: {{"repo": "owner/repo", "github_action": "list_files|fetch_file|fetch_readme", "github_path": "文件路径(fetch_file用)", "branch": "main"}} — GitHub仓库操作
- edit_file: {{"file_path": "路径", "old_text": "原文", "new_text": "新文"}} — 编辑文件
- create_directory: {{"file_path": "目录路径"}} — 创建目录
- batch_write: {{"files": [{{"path": "a.py", "content": "..."}}, ...]}} — 批量写入
- batch_edit: {{"edits": [{{"file_path": "a.py", "old_text": "...", "new_text": "..."}}, ...]}} — 批量编辑
- code_index: {{"search_pattern": "符号名", "file_path": "目录"}} — 搜索代码符号
- ast_analyze: {{"file_path": "Python文件"}} — 分析Python代码结构
- refactor: {{"refactor_action": "rename|clean_imports|analyze", "old_name": "旧名", "new_name": "新名", "file_path": "路径"}} — 代码重构
- diff_preview: {{"file_path": "路径", "old_text": "原文", "new_text": "新文"}} — 预览diff
- mcp_call: {{"tool_name": "server:tool", "tool_args": {{}}}} — 调用MCP工具
