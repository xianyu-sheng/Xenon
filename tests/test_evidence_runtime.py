...
def test_event_canonical_hash_and_copy() -> None:
    ...
    event = EvidenceEvent.create(
        session_id="run-1", phase=LifecyclePhase.UNDERSTANDING,
        kind=EventKind.TOOL_OBSERVATION, source=EvidenceSource.TOOL,
        payload=payload,
    )
...
def test_ledger_orders_links_and_queries() -> None:
    ...
    a = ledger.append(LifecyclePhase.TASK, EventKind.TASK_FACT, EvidenceSource.USER, {"goal": "fix"})
    b = ledger.append(LifecyclePhase.UNDERSTANDING, EventKind.TOOL_OBSERVATION, EvidenceSource.TOOL, {"path": "a.py"})
...
def test_ledger_rejects_tampering_and_bad_append() -> None:
    ledger = EvidenceLedger("run-1")
    event = ledger.append(LifecyclePhase.TASK, EventKind.TASK_FACT, EvidenceSource.USER, {})
    event.payload["x"] = 1
    assert not ledger.verify_integrity()
    other = EvidenceEvent.create(session_id="run-2", phase=LifecyclePhase.TASK, kind=EventKind.TASK_FACT, source=EvidenceSource.USER, payload={})
...
def test_snapshot_restore_and_jsonl_roundtrip(tmp_path: Path) -> None:
    ledger = EvidenceLedger("run-1")
    ledger.append(LifecyclePhase.TASK, EventKind.TASK_FACT, EvidenceSource.USER, {"x": 1})
    ledger.append(LifecyclePhase.VALIDATION, EventKind.GATE_VERDICT, EvidenceSource.GATE, {"passed": True})
...
def test_pack_summarizes_gates_and_requires_integrity() -> None:
    ledger = EvidenceLedger("run-1")
    ledger.append(LifecyclePhase.TASK, EventKind.TASK_FACT, EvidenceSource.USER, {"goal": "fix"})
    ledger.append(LifecyclePhase.VALIDATION, EventKind.GATE_VERDICT, EvidenceSource.GATE, {"gate": "fact", "passed": False})
...
def test_event_kind_and_phase_are_closed_protocol_values() -> None:
    with pytest.raises(ValueError):
        EvidenceEvent.create(session_id="r", phase="unknown", kind=EventKind.TASK_FACT, source=EvidenceSource.USER, payload={})
    with pytest.raises(ValueError):
        EvidenceEvent.create(session_id="r", phase=LifecyclePhase.TASK, kind="unknown", source=EvidenceSource.USER, payload={})
