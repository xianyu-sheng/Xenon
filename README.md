...
### 🆕 DeepSeek 缓存优化（v0.6.1）

专为 DeepSeek API 设计的缓存命中率追踪 + 费用估算系统。

| 能力 | 说明 |
|------|------|
| **实时监控** | 底部 toolbar 显示 💾命中率 / 💰累计费用 / 💡节省百分比 |
| **完整面板** | `/cost` 命令按模型展示调用次数、tokens、缓存 breakdown |
| **退出报告** | `/exit` 时自动打印本次会话总账单 |
| **命中率告警** | 滚动窗口对比，骤降 >40% 自动触发诊断建议 |
| **提示词对齐** | `PromptOptimizer` 自动重组消息顺序，固定前置、动态后置 |
